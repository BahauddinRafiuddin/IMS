  ...
  if (!enrollments.length) {
    return (
      <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[2.5rem] border border-dashed border-slate-200 text-center mx-4">
        ...
        <h2 className="text-xl font-bold text-slate-800">No Interns Assigned</h2>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-2 mt-1 text-xs font-bold text-slate-400 uppercase tracking-wider">
                    <span className="flex items-center gap-1.5"><Clock size={14}/> {item.program.durationInWeeks} Weeks</span>
                    <span className="flex items-center gap-1.5"><Calendar size={14}/> {new Date(item.program.startDate).toLocaleDateString()} — {new Date(item.program.endDate).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>

              <div className={`self-start md:self-center px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border shadow-sm
            ...
            <div className="bg-white rounded-4xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
               <div className="grid grid-cols-1 md:grid-cols-12 items-center p-6 gap-6">
                  ...
                  <div className="md:col-span-4 flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="font-bold text-slate-800">{item.intern.name}</h3>
                        <p className="text-[11px] font-bold text-indigo-500 uppercase tracking-tighter">Verified Intern</p>
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-3">
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">Enrolled On</p>
